...
**AUTH-02. HTML is the entrance and actionable user interface to that package.** A portable, review-named `Review-<name>.html` contains application code, bundled validators/renderers/fonts, styling, the review toolbox, and only a minimal immutable launcher binding: binding schema version, review ID, display title, and exact manifest digest. It MUST NOT contain embedded Markdown, resources, events, decisions, approvals, reviewer identity, a full manifest, or a private review database. The binding is routing/integrity metadata, never review evidence or another source of truth. The launcher works with VS Code closed and without a review server, localhost process, browser extension, CDN, or account service.
...
**AUTH-05. Client software is replaceable.** Regenerating, replacing, migrating, or removing the HTML MUST not change the interpretation or bytes of any protocol file. Client installation/upgrades MUST not create a review event or update approval state. One trusted generic build template is deterministically personalized only with the AUTH-02 binding; its executable application code remains package-independent. An explicit update MUST compare the confirmed current digest and retain the prior HTML before replacement.
...
| Install/update the review-bound HTML beside a package | Required | No self-update | Required |
...
**DECISION-04.** New packages declare initiator-managed finding control. Reviewers may comment and reply on open threads; only the immutable manifest creator ID may decide, close, or reopen them. Closing removes reply/decision actions in both clients and exposes only Reopen to the initiator; the shared write core rejects a stale submission after refreshing current state. A thread remains closed until an explicit reopen event; a decision or decision conflict cannot reopen it. This role is clearly labeled as self-asserted and MUST NOT be presented as authenticated authorization.
...
**SYNC-05.** Both the direct-folder browser and VS Code bridge automatically surface newly admitted peer events without manual refresh and visibly notify the participant. Simultaneous writers publish independent immutable files and converge by union; the UI MUST call this eventual shared-folder synchronization, not real-time messaging. Provider delivery delay, background throttling, disconnected clients and Git fetch/pull remain outside the client polling guarantee.
...
**CONTENT-04.** Structural raw Markdown HTML is supported only through the immutable `sanitized-html-v1` capability and `commonmark-gfm+sanitized-html-v1` renderer profile. Authoring, VS Code, the portable browser and audit PDF use one closed allowlist and sanitizer. Raw `<img>` and explicit attachment references become verified frozen resources; raw tables/cells receive deterministic semantic anchors; ordinary absolute links enter the reference inventory. Source scripts, styles, event handlers, active/embed/form elements and unsupported tags never execute. Profile-absent packages keep raw HTML inert, and profile-unsupported clients cannot participate.
...
**UX-01.** Opening the review-named HTML shows the expected review title immediately, explains any required first-use folder grant in plain language, verifies the selected package against the embedded review ID and exact manifest digest, and then presents its revision. A remembered directory handle is a client-local shortcut keyed by review ID and accepted only with that same binding. A replaced manifest or different selected package is refused and requires an explicit folder choice. Offer Open another, Resume, Forget connection, and a read-only mode where available.

**UX-02.** Initial directory selection and any permission request happen from a fresh user gesture. On launch, a client MAY call `queryPermission`; if read/write access is already `granted`, it SHOULD automatically open the remembered, identity-verified package without a picker. It MUST NOT call `requestPermission` or `showDirectoryPicker` automatically. When permission is `prompt`/`denied`, or automatic reconnection fails, show one explicit Resume/Choose action. Handle storage/serialization failures must not block a usable granted session. Browser file-origin support, restart persistence, and enterprise policy are verified on actual target browsers; method presence alone does not establish support. See [browser permission behavior](https://developer.chrome.com/blog/persistent-permissions-for-the-file-system-access-api) and the [File System Access draft](https://wicg.github.io/file-system-access/).
...
**UX-04.** Both UIs provide document search/navigation, an explicit revision selector, review identity, read/write/synchronization status, thread/suggestion filtering, reply composition, initiator-only decide/close/reopen, and audited export. The discussion side panel defaults to every comment and suggested edit in the pinned revision, regardless of the selected Markdown document, and offers an explicit selected-document-only scope; the chosen scope is a local view preference and never changes review state or audit scope. Comment and suggested-edit cards identify their source document and navigate to their document anchors; selecting either kind of document marker returns to its card. A resolved finding exposes only Reopen to the initiator. The current participant's stance is always visible: Approve/Reject are offered with no active stance, and only Withdraw is offered after approval or rejection. Decision values are choices, not strings reviewers must type. Conflicts, reasons, timestamps, actors, roles, and history are inspectable. Important diagnostics are persistent and accessible, not only console logs or disappearing toasts.
...
**UX-07.** Review actions remain reachable after deep scrolling in a long document. The primary toolbar stays visible, and selecting frozen text or a semantic object presents contextual Comment/Suggest actions near the viewport without changing the anchor or duplicating document content. The contextual control is keyboard reachable, responsive, and never obscures the only route to an action.
...
**EXPORT-02.** The PDF is a self-contained human-readable **offline review archive**, not a printout of the current screen. It contains all selected-revision documents in deterministic order, frozen images, readable tables, Mermaid, attachment inventory, external references, comments/replies, all decisions and reasons, suggestion changes/lifecycle, approvals/rejections/withdrawals, actor assurance, policy result, verification cutoff, diagnostics, and renderer versions. Export is independent of the currently visible document, filters, or collapsed discussion cards. Live external references that were not captured are visibly labeled as unavailable offline; they are never silently presented as archived evidence.

**EXPORT-02A.** The archive has five explicit human sections: review challenge register; frozen review documents with a per-document challenge map; review comments, responses, and challenged topics; review stances and outcome; and the audit appendix/exact file inventory. Every root comment and suggested edit receives one deterministic archive-local finding reference in document/location order. The reference is not a protocol identity and MAY be renumbered in an export with a different observed event set. Each challenge-map entry shows current status, exact document/line/character, and a bounded quote; it links to the complete finding record, which links back when the PDF engine supports internal destinations.

**EXPORT-02B.** Full review discussion belongs in the dedicated comments section rather than being duplicated throughout document rendering. A comment finding shows the frozen anchor quote, initial comment, every reply, complete decision and close/reopen history, actors, timestamps, reasons, and current open/resolved/conflict status. A suggested-edit finding shows the original text, proposed insertion/replacement/deletion, rationale, complete accept/reject/application history, actors, timestamps, reasons, and current status. The stance section shows each participant's complete approve/reject/withdraw history and current status. The archive summary makes it possible to determine offline what topics were challenged and whether they remain open.
...
**EXPORT-06.** The PDF is derived evidence and MUST NOT become a second source of truth. Archive-local finding labels, hyperlinks, headings, summaries, and page numbers are derived entirely from the frozen revision and admitted events named by the detached inventory. Comments are never edited or stored inside the PDF as authoritative annotations. Regenerating an archive does not append review actions other than the detached `export.created` evidence event.

## 10. Authoring service and CLI contract

**CREATE-01.** Graphical setup selects a source root, individual files/folders, exclusions, start document, title, actor, optional completion policy, and package location/name. Every successful creation installs the review-named, AUTH-02-bound HTML entry file, including its bundled JavaScript, Mermaid and PDF runtime dependencies. The same inputs are accepted as CLI parameters. Normal disks, user-mounted shares, and Git folders are first-class locations; no storage-provider account is required.

**CREATE-02.** New creation requires a new or empty target directory. It never overwrites an existing manifest or silently adds another review to an existing package. The service resolves selections to exact, portable Markdown paths; captures blobs/policy/descriptor; publishes the first revision last; then installs the review-bound HTML. A failed capture yields a recoverable incomplete initialization, not a published review. Client installation failure after publication yields an existing valid review with a distinct client-installation error.
...
**CREATE-07. Private source linkage is explicit.** A received, moved, or reopened package MUST NOT infer an editable source from the first open workspace. VS Code may retain a private mapping from review ID/package connection to one native source root. Linking enumerates every document in the current revision head set and reports exact, changed, and missing paths; a missing current document prevents author-only source actions. Changed bytes are allowed because a later revision is an explicit new snapshot. The native root is never stored in the package, HTML, events, revisions, exports, or audit inventory. Source application and new-revision creation remain disabled until the user deliberately links or supplies a source.

**CREATE-08. Package publication/relocation is verified transport, not review history.** A native client may publish a complete package to a different empty local, mounted, synchronized, or Git folder. It MUST reject links/special entries, nested source/destination roots, unknown top-level content and nonempty destinations; inventory and hash every file; copy with exclusive destination creation; write `manifest.json` only after all other files close; re-inventory both sides; and fully validate the destination before switching the active connection. A source change during transport prevents activation. No review event, new review ID, manifest change, or source path is created. The original is never automatically deleted and is clearly detached locally. Other machines/processes cannot be locked, so the UI requires writers to stop and does not call a changing copy complete.

**CREATE-09. Markdown scope has one explicit source root in protocol 0.5.** Multi-root VS Code workspaces require choosing one folder. Additional roots grant capture of referenced resources only. Supporting Markdown documents from unrelated roots requires a separately versioned namespaced-source capability with portable aliases; implementations MUST NOT place absolute native roots or ambiguous duplicate paths into the 0.5 document namespace.
...
**SEC-03.** Undeclared raw Markdown HTML is disabled. The declared [sanitized HTML profile](../protocol/profiles/sanitized-html-v1.md) reconstructs only a closed structural subset, strips every non-allowlisted attribute, escapes unsupported tags, freezes image/attachment bytes and permits no review-supplied script, style, SVG DOM, form, frame, media or live image. Mermaid is strict and sanitized. SVG is sanitized for inline use or rendered in a non-scriptable image context. Attachment MIME and content are validated; active content is never opened in the application's scripting origin. Unsupported files can be safely downloaded with a clear label where policy permits, but required unsupported evidence blocks publication. HTTPS redirects are bounded and revalidated at every hop; HTTP downgrade and embedded credentials are rejected.
...
**SEC-05.** Version/hash metadata identifies the HTML build; the AUTH-02 binding identifies its intended immutable manifest without embedding review state. `client update` verifies a trusted installed release artifact, updates only the recognized generated HTML, and preserves a recoverable prior client file outside protocol evidence directories. Unknown/user-modified HTML requires an explicit replacement choice. Updating executable HTML on a shared folder must be announced as a client update. A checksum stored beside modifiable software is not an independent trust anchor.
